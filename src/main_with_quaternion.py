...
# TODO: pip install numpy-quaternion
...
class BalancingRobot:
    ...
    def _calculate_angle(self, data, dt):
        """Calculate orientation using quaternions."""
        ...
        # Initialize quaternion if not already done
        if not hasattr(self, 'quaternion'):
            self.quaternion = np.quaternion(1, 0, 0, 0)  # Identity quaternion

        # Update quaternion using gyroscope data (integration)
        delta_q = np.quaternion(0, gx * dt, gy * dt, gz * dt) * self.quaternion * 0.5
        self.quaternion += delta_q
